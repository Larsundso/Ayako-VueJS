...
const Helper = {
  ...
  getMime: (u: url.UrlWithParsedQuery): string => {
    const ext = Helper.getExt(u.pathname)?.replace('.', '');
    return ext
      ? Helper.types[ext.toLowerCase() as keyof typeof Helper.types] || 'application/octet-stream'
      : 'application/octet-stream';
  },

  getExt: (path: string | null) => {
